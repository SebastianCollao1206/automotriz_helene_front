...
export class CategoriasModal implements OnChanges {

  @Input() mostrar: boolean = false;
  @Input() modoEdicion: boolean = false;
  @Input() categoriaData?: CategoriaResponse;

  @Output() alCerrar = new EventEmitter<void>();
  @Output() alGuardar = new EventEmitter<{ data: CrearCategoriaRequest | ActualizarCategoriaRequest, id?: number }>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['mostrar'] && this.mostrar) {
      if (this.modoEdicion && this.categoriaData) {
        this.cargarDatos();
      } else {
  ...
  private cargarDatos(): void {
    if (!this.categoriaData) return;

    this.formulario.patchValue({
      nombre: this.categoriaData.nombre,
      enabled: this.categoriaData.enabled
    });
  ...
  enviarFormulario(): void {
    ...
    if (this.modoEdicion && this.categoriaData) {
      ...
      this.alGuardar.emit({ data: request, id: this.categoriaData.id });
    } else {
