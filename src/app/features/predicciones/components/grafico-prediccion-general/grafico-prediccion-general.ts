...
@Component({
  selector: 'app-grafico-prediccion-general',
  imports: [CommonModule, BaseChartDirective],
  templateUrl: './grafico-prediccion-general.html',
  styleUrl: './grafico-prediccion-general.css'
})
export class GraficoPrediccionGeneral implements OnInit {
  ...
  private cargarDatosGrafico(): void {
    ...
    this.prediccionService.obtenerGraficoGeneral()
      .pipe(finalize(() => {
  ...
  private configurarGrafico(): void {
    ...
    todosLosDatos.sort((a: any, b: any) => {
    ...
    const labels = todosLosDatos.map((d: any) => d.mes);

    const datosHistoricos = new Array(labels.length).fill(null);
    this.datosGrafico.datosHistoricos.forEach((d: any) => {
    ...
    this.datosGrafico.datosPrediccion.forEach((d: any) => {
      const index = labels.indexOf(d.mes);
      if (index !== -1) {
    ...
    this.lineChartData = {
      ...
      datasets: [
        ...
        },
        {
          ...
          tension: 0.4,
          spanGaps: true,
          pointRadius: datosPrediccion.map((val: any, idx: number) => {
            const primerIndicePrediccion = datosPrediccion.findIndex((v: any) => v !== null);
